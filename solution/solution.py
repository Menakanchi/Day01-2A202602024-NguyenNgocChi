...
def run_assistant(
    ...
    while True:
        ...
        if not user_msg or user_msg.strip().lower() in ("quit", "exit", "bye"):
            break
        ...
        turns += 1
        cost_dict = estimate_cost(user_msg, reply, model=OPENAI_MODEL)
...
def format_comparison_table(results: list[dict]) -> str:
    ...
    for r in results:
        ...
        g_ans = (r.get("gpt4o_answer", "")[:37] + "...") if len(r.get("gpt4o_answer", "")) > 40 else r.get("gpt4o_answer", "")
        m_ans = (r.get("mini_answer", "")[:37] + "...") if len(r.get("mini_answer", "")) > 40 else r.get("mini_answer", "")
        g_lat = f"{r.get('gpt4o_time', 0.0):.2f}s"
        m_lat = f"{r.get('mini_time', 0.0):.2f}s"
    return "".join(lines)
